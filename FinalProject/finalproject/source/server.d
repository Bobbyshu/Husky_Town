...
class TCPServer{
    ...
    Socket[string] mClientSocketForXY;
    ...
    this(string host = "localhost", ushort port=50001, ushort portForXY=50002, ushort maxConnectionsBacklog=100){
        ...
        writeln("Packet Server address: ", host, ":", portForXY);
    ...
    /**
     * send history message for new client
     */
    private void sendHistoryMessages(Socket clientSocket) {
        foreach (msg; mServerData) {
            clientSocket.send(msg[0 .. 1024]);
        }
    }
    ...
    private void spawnListenPlayerCoordinateSocket() {
        while (true) {
            ...
            string clientAddress = to!string(newClientSocketForXY.remoteAddress());
            
            mClientSocketForXY[clientAddress] = newClientSocketForXY;

            mCurrentMessageToSend ~= 0;

            writeln("Friends on server = ", mClientSocketForXY.length);
    ...
    void clientLoop(Socket clientSocket){
        ...
        while(runThreadLoop){
            ...
            if (!hasReceivedHistory) {
                sendHistoryMessages(clientSocket);
            ...
            if (receivedData.startsWith("FILE_TRANSFER")) {
                ...
                mServerData ~= buffer;
                broadcastToAllClients(clientSocket);
            }
        }
    ...
    void xyClientLoop(Socket clientSocket) {
        ...
        while (runThreadLoop) {
            ...
            if(!clientSocket.isAlive){
                runThreadLoop=false;
                string remoteAddress = to!string(clientSocket.remoteAddress());
                string clientName = clientPostMap[remoteAddress];
                writeln("XY client disconnected or error occurred. Client address: " ~ remoteAddress ~ " name " ~ clientName);
                runThreadLoop = false;
                map[clientName] = [-1, -1, 1];
                mClientSocketForXY.remove(remoteAddress);
                
                Packet packet;
                packet.name = clientName.dup;
                packet.x = -1;
                packet.y = -1;
                if(map.length==0){
                    packet.characternum=1;
                }else{
                    packet.characternum = map.length%5;
                }
                
                auto packetBytes = packet.GetPacketAsBytes();
            ...
            clientPostMap[to!string(clientSocket.remoteAddress())] = to!string(p.name);
        ...
        writeln("exiting the XY Client loop");
        string remoteAddress = to!string(clientSocket.remoteAddress());
        string clientName = clientPostMap[remoteAddress];
        map[clientName] = [-1, -1, 1];
    }
     ...
     */
    void broadcastToAllClients(Socket senderSocket){
        ...
        foreach(idx,serverToClient; mClientsConnectedToServer){
            ...
            while(mCurrentMessageToSend[idx] <= mServerData.length-1){
                if (senderSocket != serverToClient) {
